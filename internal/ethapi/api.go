...
<<<<<<< HEAD
	"encoding/json"
=======
>>>>>>> f28b98a9947b52816714f90326b2545b08606b1a
	"errors"
	"fmt"
	"math/big"
	"strings"
	...
	for i, tx := range txs {
		...
		if result.Err != nil {
			...
			if len(revert) > 0 {
				...
				// also add revertBytes and add the revertBytes as hex bytes like 0x00000....123344000
				dst := make([]byte, hex.EncodedLen(len(revert)))
				hex.Encode(dst, revert)
				jsonResult["revertBytes"] = "0x" + string(dst)
			}
		} else {
